...
try:
    with sync_playwright() as p:
        ...
        try:
            ...
            # التقاط صورة لنرى هل نجح البروكسي في تخطي شاشة الهاتف
            page.screenshot(path="proxy_step.png")
            send_tg_photo("proxy_step.png", "📸 حالة الدخول باستخدام البروكسي")
            ...
            if match:
                ...
                final_link = f"vless://{USER_UUID}@{SNI_URL}:443?encryption=none&security=tls&sni={SNI_URL}&type=ws&host={url_v}&path=%2F#Proxy-Success"
                send_tg(f"✅ مبروك! البروكسي نجح في المهمة:\n\n`{final_link}`")
            else:
                page.screenshot(path="final_check.png")
                send_tg_photo("final_check.png", "⚠️ اكتمل البناء لكن لم أجد الرابط النهائي.")
                
        except Exception as inner_e:
            page.screenshot(path="error.png")
            send_tg_photo("error.png", f"❌ توقف المتصفح (حتى مع البروكسي):\n{str(inner_e)[:100]}")
        finally:
            browser.close()
except Exception as e:
    send_tg(f"❌ خطأ عام في الاتصال بالبروكسي:\n{str(e)[:100]}")
